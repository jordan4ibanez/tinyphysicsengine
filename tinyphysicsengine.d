module tinyphysicsengine;
@nogc nothrow:
extern(C): __gshared:
#ifndef _TINYPHYSICSENGINE_H
version = _TINYPHYSICSENGINE_H;
...
alias int TPE_Unit;               ///< Basic fixed point unit type.
alias short TPE_UnitReduced;        ///< Like TPE_Unit but saving space

enum TPE_FRACTIONS_PER_UNIT = 512      ///< one fixed point unit, don't change;
enum TPE_F = TPE_FRACTIONS_PER_UNIT    ///< short for TPE_FRACTIONS_PER_UNIT;
enum TPE_JOINT_SIZE_MULTIPLIER = 32    ///< joint size is scaled (size saving);

enum TPE_INFINITY = 2147483647;

enum string TPE_JOINT_SIZE(string joint) = ` ((joint).sizeDivided * TPE_JOINT_SIZE_MULTIPLIER)`;

version (TPE_APPROXIMATE_LENGTH) {} else {
  enum TPE_APPROXIMATE_LENGTH = 0      /**< whether or not use length/distance ;
                                           approximation rather; than exact; 
                                           calculation (1 is_ faster but less
                                           accurate), beware of possible; lower
                                           stability */
}

static if (!TPE_APPROXIMATE_LENGTH) {
  enum TPE_DISTANCE = TPE_dist;
  enum TPE_LENGTH = TPE_vec3Len;
} else {
  enum TPE_DISTANCE = TPE_distApprox;
  enum TPE_LENGTH = TPE_vec3LenApprox;
}

version (TPE_LOG) {} else {
  enum string TPE_LOG(string s) = ` ; // redefine to some print function to show debug logs`;
}

version (TPE_LOW_SPEED) {} else {
/** Speed, in TPE_Units per ticks, that is considered low (used e.g. for auto
  deactivation of bodies). */
  enum TPE_LOW_SPEED = 30;
}

version (TPE_RESHAPE_TENSION_LIMIT) {} else {
/** Tension limit, in TPE_Units, after which a non-soft body will be reshaped.
  ...
  enum TPE_RESHAPE_TENSION_LIMIT = 20;
}

version (TPE_RESHAPE_ITERATIONS) {} else {
/** How many iterations of reshaping will be performed by the step function if
  ...
  enum TPE_RESHAPE_ITERATIONS = 3;
}

version (TPE_DEACTIVATE_AFTER) {} else {
/** After how many ticks of low speed should a body be disabled. This mustn't
  be greater than 255. */
  enum TPE_DEACTIVATE_AFTER = 128;
}

version (TPE_LIGHT_DEACTIVATION) {} else {
/** When a body is activated by a collision, its deactivation counter will be
  ...
  rest. */
  enum TPE_LIGHT_DEACTIVATION = \
    (TPE_DEACTIVATE_AFTER - TPE_DEACTIVATE_AFTER / 10);
}

version (TPE_TENSION_ACCELERATION_DIVIDER) {} else {
/** Number by which the base acceleration (TPE_FRACTIONS_PER_UNIT per tick
  ...
  power of 2. */
  enum TPE_TENSION_ACCELERATION_DIVIDER = 32;
}

version (TPE_TENSION_ACCELERATION_THRESHOLD) {} else {
/** Limit within which acceleration caused by connection tension won't be
  applied. */
  enum TPE_TENSION_ACCELERATION_THRESHOLD = 5;
}

version (TPE_TENSION_GREATER_ACCELERATION_THRESHOLD) {} else {
/** Connection tension threshold after which twice as much acceleration will
  ...
  environment.*/
  enum TPE_TENSION_GREATER_ACCELERATION_THRESHOLD = \
    (TPE_TENSION_ACCELERATION_THRESHOLD * 3);
}

version (TPE_COLLISION_RESOLUTION_ITERATIONS) {} else {
/** Maximum number of iterations to try to uncollide two colliding bodies. */
  enum TPE_COLLISION_RESOLUTION_ITERATIONS = 16;
}

version (TPE_COLLISION_RESOLUTION_MARGIN) {} else {
/** Margin, in TPE_Units, by which a body will be shifted back to get out of
  collision. */
  enum TPE_COLLISION_RESOLUTION_MARGIN = (TPE_F / 64);
}

version (TPE_NONROTATING_COLLISION_RESOLVE_ATTEMPTS) {} else {
/** Number of times a collision of nonrotating bodies with environment will be
  ...
  enum TPE_NONROTATING_COLLISION_RESOLVE_ATTEMPTS = 8;
}

version (TPE_APPROXIMATE_NET_SPEED) {} else {
/** Whether to use a fast approximation for calculating net speed of bodies
  which increases performance a bit. */
  enum TPE_APPROXIMATE_NET_SPEED = 1;
}

enum string TPE_PRINTF_VEC3(string v) = ` printf("[%d %d %d]",(v).x,(v).y,(v).z);`;

struct _TPE_Vec3 {
  TPE_Unit x;
  TPE_Unit y;
  TPE_Unit z;
}alias _TPE_Vec3 TPE_Vec3;
...
TPE_Vec3 TPE_vec3KeepWithinBox(TPE_Vec3 point, TPE_Vec3 boxCenter, TPE_Vec3 boxMaxVect);

TPE_Vec3 TPE_vec3KeepWithinDistanceBand(TPE_Vec3 point, TPE_Vec3 center, TPE_Unit minDistance, TPE_Unit maxDistance);
...
pragma(inline, true) private TPE_Unit TPE_abs(TPE_Unit x);
pragma(inline, true) private TPE_Unit TPE_max(TPE_Unit a, TPE_Unit b);
pragma(inline, true) private TPE_Unit TPE_min(TPE_Unit a, TPE_Unit b);
pragma(inline, true) private TPE_Unit TPE_nonZero(TPE_Unit x);
pragma(inline, true) private TPE_Unit TPE_dist(TPE_Vec3 p1, TPE_Vec3 p2);
pragma(inline, true) private TPE_Unit TPE_distApprox(TPE_Vec3 p1, TPE_Vec3 p2);
...
struct _TPE_Joint {
  TPE_Vec3 position;
  TPE_UnitReduced[3] velocity; ///< not TPE_Vec3 to save size
  ubyte sizeDivided; /**< size (radius, ...), for saving space divided by 
                            TPE_JOINT_SIZE_MULTIPLIER */
}alias _TPE_Joint TPE_Joint;

struct _TPE_Connection {
  ubyte joint1;
  ubyte joint2;
  ushort length;     ///< connection's preferred length, uint16_t saves space
}alias _TPE_Connection TPE_Connection;

enum TPE_BODY_FLAG_DEACTIVATED = 1    /**< Not being updated due to low energy,;
                                            "sleeping", will be woken; by collisions; etc. */
enum TPE_BODY_FLAG_NONROTATING = 2    /**< When set, the body won't rotate, ;
                                            will only; move whole;
                                            body. */
enum TPE_BODY_FLAG_DISABLED = 4       /**< Disabled, not taking part in;
                                            simulation. */
enum TPE_BODY_FLAG_SOFT = 8           /**< Soft connections, effort won't be;
                                            made to; keep the body's shape. */
enum TPE_BODY_FLAG_SIMPLE_CONN = 16   /**< Simple connections, don't zero out;
                                            antagonist forces; or apply;
                                            connection friction;connection can; increase
                                            performance. */
enum TPE_BODY_FLAG_ALWAYS_ACTIVE = 32 /**< Will never deactivate due to low;
                                            energy. */
...
alias ubyte function(ushort, ushort, ushort, ushort, TPE_Vec3) TPE_CollisionCallback;
...
alias void function(ushort, ushort, ubyte) TPE_DebugDrawFunction;
...
struct _TPE_Body {
  ...
  ubyte flags;
  ubyte deactivateCount;
}alias _TPE_Body TPE_Body;
...
ubyte TPE_testClosestPointFunction(TPE_ClosestPointFunction f, TPE_Vec3 cornerFrom, TPE_Vec3 cornerTo, ubyte gridResolution, TPE_UnitReduced allowedError, TPE_Vec3* errorPoint);

void TPE_bodyInit(TPE_Body* body, TPE_Joint* joints, ubyte jointCount, TPE_Connection* connections, ubyte connectionCount, TPE_Unit mass);

void TPE_worldInit(TPE_World* world, TPE_Body* bodies, ushort bodyCount, TPE_ClosestPointFunction environmentFunction);
...
TPE_Vec3 TPE_bodyGetRotation(const(TPE_Body)* body, ushort joint1, ushort joint2, ushort joint3);
...
pragma(inline, true) private TPE_Unit TPE_connectionTension(TPE_Unit length, TPE_Unit desiredLength);
...
void TPE_getVelocitiesAfterCollision(TPE_Unit* v1, TPE_Unit* v2, TPE_Unit m1, TPE_Unit m2, TPE_Unit elasticity);
...
ubyte TPE_jointsResolveCollision(TPE_Joint* j1, TPE_Joint* j2, TPE_Unit mass1, TPE_Unit mass2, TPE_Unit elasticity, TPE_Unit friction, TPE_ClosestPointFunction env);
...
ubyte TPE_jointEnvironmentResolveCollision(TPE_Joint* joint, TPE_Unit elasticity, TPE_Unit friction, TPE_ClosestPointFunction env);

/** Tests whether a body is currently colliding with the environment. */
ubyte TPE_bodyEnvironmentCollide(const(TPE_Body)* body, TPE_ClosestPointFunction env);
...
ubyte TPE_bodyEnvironmentResolveCollision(TPE_Body* body, TPE_ClosestPointFunction env);
...
void TPE_bodyGetFastBSphere(const(TPE_Body)* body, TPE_Vec3* center, TPE_Unit* radius);

/** Computes the minimum bounding sphere of a body (there is another function
  for a faster approximate bounding sphere). */
void TPE_bodyGetBSphere(const(TPE_Body)* body, TPE_Vec3* center, TPE_Unit* radius);

ubyte TPE_checkOverlapAABB(TPE_Vec3 v1Min, TPE_Vec3 v1Max, TPE_Vec3 v2Min, TPE_Vec3 v2Max);
...
ubyte TPE_bodiesResolveCollision(TPE_Body* b1, TPE_Body* b2, TPE_ClosestPointFunction env);
...
/** "Fakes" a rotation of a moving sphere by rotating it in the direction of
  ...
  library conventions). */
TPE_Vec3 TPE_fakeSphereRotation(TPE_Vec3 position1, TPE_Vec3 position2, TPE_Unit radius);
...
/** Casts a ray against environment and returns the closest hit of a surface. If
  ...
  surface. */
TPE_Vec3 TPE_castEnvironmentRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, TPE_ClosestPointFunction environment, TPE_Unit insideStepSize, TPE_Unit rayMarchMaxStep, uint maxSteps);
...
/** Casts a ray against bodies in a world (ignoring the environment), returns
  ...
  ignored). */
TPE_Vec3 TPE_castBodyRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, short excludeBody, const(TPE_World)* world, short* bodyIndex, short* jointIndex);
...
void TPE_bodyLimitAverageSpeed(TPE_Body* body, TPE_Unit speedMin, TPE_Unit speedMax);

/** Deactivates a body (puts it to sleep until another collision or force wake
  up). */
void TPE_bodyDeactivate(TPE_Body* body);

pragma(inline, true) private ubyte TPE_bodyIsActive(const(TPE_Body)* body);
...
void TPE_bodyReshape(TPE_Body* body, TPE_ClosestPointFunction environmentFunction);
...
/** Mostly for internal use, performs some "magic" on body connections, mainly
  ...
  body is soft or not. */
void TPE_bodyCancelOutVelocities(TPE_Body* body, ubyte strong);
...
/** Draws a debug view of a 3D physics world using a provided pixel drawing
  ...
  e.g. 16). */
void TPE_worldDebugDraw(TPE_World* world, TPE_DebugDrawFunction drawFunc, TPE_Vec3 camPos, TPE_Vec3 camRot, TPE_Vec3 camView, ushort envGridRes, TPE_Unit envGridSize);

enum TPE_DEBUG_COLOR_CONNECTION = 0;
enum TPE_DEBUG_COLOR_JOINT = 1;
enum TPE_DEBUG_COLOR_ENVIRONMENT = 2;
enum TPE_DEBUG_COLOR_INACTIVE = 3;

uint TPE_jointHash(const(TPE_Joint)* joint);
uint TPE_connectionHash(const(TPE_Connection)* connection);
uint TPE_bodyHash(const(TPE_Body)* body);
...
uint TPE_worldHash(const(TPE_World)* world);

// FUNCTIONS FOR GENERATING BODIES

void TPE_makeBox(TPE_Joint[8] joints, TPE_Connection[16] connections, TPE_Unit width, TPE_Unit depth, TPE_Unit height, TPE_Unit jointSize);
void TPE_makeCenterBox(TPE_Joint[9] joints, TPE_Connection[18] connections, TPE_Unit width, TPE_Unit depth, TPE_Unit height, TPE_Unit jointSize);
void TPE_makeRect(TPE_Joint[4] joints, TPE_Connection[6] connections, TPE_Unit width, TPE_Unit depth, TPE_Unit jointSize);
void TPE_makeTriangle(TPE_Joint[3] joints, TPE_Connection[3] connections, TPE_Unit sideLength, TPE_Unit jointSize);
void TPE_makeCenterRect(TPE_Joint[5] joints, TPE_Connection[8] connections, TPE_Unit width, TPE_Unit depth, TPE_Unit jointSize);
void TPE_makeCenterRectFull(TPE_Joint[5] joints, TPE_Connection[10] connections, TPE_Unit width, TPE_Unit depth, TPE_Unit jointSize);
void TPE_make2Line(TPE_Joint[2] joints, TPE_Connection[1] connections, TPE_Unit length, TPE_Unit jointSize);
...
TPE_Vec3 TPE_envBox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec, TPE_Vec3 rotation);
...
TPE_Vec3 TPE_envInfiniteCylinder(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 direction, TPE_Unit radius);
TPE_Vec3 TPE_envCylinder(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 direction, TPE_Unit radius);
TPE_Vec3 TPE_envCone(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 direction, TPE_Unit radius);
TPE_Vec3 TPE_envLineSegment(TPE_Vec3 point, TPE_Vec3 a, TPE_Vec3 b);
TPE_Vec3 TPE_envHeightmap(TPE_Vec3 point, TPE_Vec3 center, TPE_Unit gridSize, TPE_Unit function(int x, int y) heightFunction, TPE_Unit maxDist);
...
/** Environment function for triagnular prism, e.g. for ramps. The sides array
  ...
  2).*/
TPE_Vec3 TPE_envAATriPrism(TPE_Vec3 point, TPE_Vec3 center, const(TPE_Unit)[6] sides, TPE_Unit depth, ubyte direction);
...
enum string TPE_ENV_START(string test,string point) = ` TPE_Vec3 _pBest = test, _pTest; \
  TPE_Unit _dBest = TPE_DISTANCE(_pBest,point), _dTest; \
  (void)(_pBest); (void)(_dBest); (void)(_dTest); (void)(_pTest); // supress war`;

enum string TPE_ENV_NEXT(string test,string point) = ` \
...
enum TPE_ENV_END = return _pBest;;

enum string TPE_ENV_BCUBE_TEST(string bodyBCubeC,string bodyBCubeR,string envBCubeC,string envBCubeR) = ` ( \
...
enum string TPE_ENV_BSPHERE_TEST(string bodyBSphereC,string bodyBSphereR,string envBSphereC,string envBSphereR) = `\
  (TPE_DISTANCE(bodyBSphereC,envBSphereC) <= ((bodyBSphereR) + (envBSphereR)))`;

//------------------------------------------------------------------------------
// privates:

ushort _TPE_body1Index;ushort _TPE_body2Index;ushort _TPE_joint1Index;ushort _TPE_joint2Index;
...
enum string C(string n,string a,string b) = ` connections[n].joint1 = a; connections[n].joint2 = b;`;
...
void TPE_worldStep(TPE_World* world) {
  ...
  for (ushort i = 0; i < world.bodyCount; ++i)
  {
    ...
    TPE_Joint* joint = body.joints;TPE_Joint* joint2;

    TPE_Vec3 origPos = body.joints[0].position;

    for (ushort j = 0; j < body.jointCount; ++j) // apply velocities
    {
    ...
    TPE_Vec3 aabbMin;TPE_Vec3 aabbMax;
    ...
    ubyte collided = TPE_bodyEnvironmentResolveCollision(body,world.environmentFunction);
    ...
    if (body.flags & TPE_BODY_FLAG_NONROTATING)
    {
      ...
      for (ubyte i = 0; i < TPE_NONROTATING_COLLISION_RESOLVE_ATTEMPTS; ++i) 
      {
        if (!collided)
          break;
    ...
    else // normal, rotating bodies
    {
      ...
      for (ushort j = 0; j < body.connectionCount; ++j) // joint tension
      {
      ...
      {
        ubyte hard = !(body.flags & TPE_BODY_FLAG_SOFT);
        ...
        if (hard)
        {
          ...
          if (bodyTension > TPE_RESHAPE_TENSION_LIMIT)
            for (ubyte k = 0; k < TPE_RESHAPE_ITERATIONS; ++k)
              TPE_bodyReshape(body,world.environmentFunction);
        }
...
TPE_Unit TPE_bodyGetNetSpeed(const(TPE_Body)* body) {
static if (TPE_APPROXIMATE_NET_SPEED) {
  TPE_Vec3 netV = TPE_vec3(0,0,0);
    
  const(TPE_Joint)* joint = body.joints;

  for (ushort i = 0; i < body.jointCount; ++i)
  {
...
void TPE_bodyMultiplyNetSpeed(TPE_Body* body, TPE_Unit factor) {
  ...
  for (ushort j = 0; j < body.jointCount; ++j)
  {
    for (ubyte k = 0; k < 3; ++k)
      joint.velocity[k] = 
        ((cast(TPE_Unit) joint.velocity[k]) * factor) /
        TPE_F;
...
void TPE_bodyLimitAverageSpeed(TPE_Body* body, TPE_Unit speedMin, TPE_Unit speedMax) {
  for (ubyte i = 0; i < 16; ++i)
  {
    ...
    TPE_Unit fraction = (((speedMax + speedMin) / 2) * TPE_F) /
      TPE_nonZero(speed);
...
void TPE_bodyCancelOutVelocities(TPE_Body* body, ubyte strong) {
  for (ushort i = 0; i < body.connectionCount; ++i)
  {
    ...
    if (cancel)
    {
      TPE_Vec3 v1 = TPE_vec3(j1.velocity[0],j1.velocity[1],j1.velocity[2]);TPE_Vec3 v2 = TPE_vec3(j2.velocity[0],j2.velocity[1],j2.velocity[2]);
...
void _TPE_bodyNonrotatingJointCollided(TPE_Body* b, short jointIndex, TPE_Vec3 origPos, ubyte success) {
  origPos = TPE_vec3Minus(b.joints[jointIndex].position,origPos);

  for (ushort i = 0; i < b.jointCount; ++i)
    if (i != jointIndex)
    {
...
TPE_Unit TPE_sin(TPE_Unit x) {
  ...
  enum _PI2 = 5053 // 9.8696044 * TPE_F;
...
ubyte TPE_bodiesResolveCollision(TPE_Body* b1, TPE_Body* b2, TPE_ClosestPointFunction env) {
  ...
  for (ushort i = 0; i < b1.jointCount; ++i)
    for (ushort j = 0; j < b2.jointCount; ++j)
    {
...
ubyte TPE_jointsResolveCollision(TPE_Joint* j1, TPE_Joint* j2,
  ...
  TPE_ClosestPointFunction env)
{
  ...
  if (d < 0) // collision?
  {
    ...
    TPE_Vec3 pos1Backup = j1.position;TPE_Vec3 pos2Backup = j2.position;
    ...
    TPE_Vec3 frictionVec = TPE_vec3(j1.velocity[0],j1.velocity[1],j1.velocity[2]);
...
enum string assignVec(string j,string i,string d,string o) = ` \
...
ubyte TPE_jointEnvironmentResolveCollision(TPE_Joint *joint,
  TPE_Unit elasticity, TPE_Unit friction, TPE_ClosestPointFunction env)
{
  ...
  if (len <= TPE_JOINT_SIZE(*joint))
  {
    ...
    TPE_Vec3 positionBackup = joint.position;TPE_Vec3 shift;
    ubyte success = 0;
...
void TPE_bodyGetBSphere(const(TPE_Body)* body, TPE_Vec3* center, TPE_Unit* radius) {
  ...
  *center = TPE_bodyGetCenterOfMass(body);
  ...
  for (ushort i = 0; i < body.jointCount; ++i)
  {
    ...
    TPE_Unit distSquared = diff.x * diff.x + diff.y * diff.y + diff.z * diff.z;
...
ubyte TPE_bodyEnvironmentResolveCollision(TPE_Body* body, TPE_ClosestPointFunction env) {
  ...
  for (ushort i = 0; i < body.jointCount; ++i)
  {
    ...
    ubyte r = TPE_jointEnvironmentResolveCollision(
...
TPE_Unit TPE_atan(TPE_Unit x) {
  ...
  TPE_Unit sign = 1;TPE_Unit x2 = x * x;
...
TPE_Vec3 _TPE_project3DPoint(TPE_Vec3 p, TPE_Vec3 camPos, TPE_Vec3 camRot, TPE_Vec3 camView) {
...
void _TPE_drawDebugPixel(TPE_Unit x, TPE_Unit y, TPE_Unit w, TPE_Unit h, ubyte c, TPE_DebugDrawFunction f) {
  if (x >= 0 && x < w && y >= 0 && y < h)
    f(x,y,c);
}
...
enum Z_LIMIT = 250;
  if (world.environmentFunction != 0)
  {
    ...
    for (ubyte j = 0; j < envGridRes; ++j)
    {
      testPoint.x = center.x - gridHalfSize;

      for (ubyte k = 0; k < envGridRes; ++k)
      {
        testPoint.z = center.z - gridHalfSize;

        for (ubyte l = 0; l < envGridRes; ++l)
        {
  ...
  {
    // connections:
    for (ushort j = 0; j < world.bodies[i].connectionCount; ++j)
    {
      TPE_Vec3 p1 = world.bodies[i].joints[
          world.bodies[i].connections[j].joint1].position;TPE_Vec3 p2 = world.bodies[i].joints[
...
enum SEGS = 16;
      ...
      ubyte c = (world.bodies[i].flags & TPE_BODY_FLAG_DEACTIVATED) ?
        TPE_DEBUG_COLOR_INACTIVE : TPE_DEBUG_COLOR_CONNECTION;

      for (ushort k = 0; k < SEGS; ++k)
      {
    ...
    // joints:
    for (ushort j = 0; j < world.bodies[i].jointCount; ++j)
    {
      ...
      if (p.z > Z_LIMIT)
      {
        ubyte color = (world.bodies[i].flags & TPE_BODY_FLAG_DEACTIVATED) ?
...
enum SEGS = 4;
        for (ubyte k = 0; k < SEGS + 1; ++k)
        {
          TPE_Unit dx = (TPE_sin(TPE_F * k / (8 * SEGS)) * size)
              / TPE_F;TPE_Unit dy = (TPE_cos(TPE_F * k / (8 * SEGS)) * size)
              / TPE_F;

enum string dp(string a,string b,string c,string d) = ` \
...
TPE_Vec3 TPE_envAABox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec) {
  ...
  ubyte region = (shifted.x > maxCornerVec.x) |
    ((shifted.y > maxCornerVec.y) << 1) |
    ((shifted.z > maxCornerVec.z) << 2);

  switch (region)
  {
enum string align_(string c,string i) = ` point.c = center.c + sign[i] * maxCornerVec.c`;

    case 0x01: align_(x,0); break;
    case 0x02: align_(y,1); break;
    case 0x04: align_(z,2); break;

    case 0x03: align_(x,0); align_(y,1); break;
    case 0x05: align_(x,0); align_(z,2); break;
    case 0x06: align_(y,1); align_(z,2); break;

    case 0x07: align_(x,0); align_(y,1); align_(z,2); break; 
    default: break;
...
TPE_Vec3 TPE_envAABoxInside(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 size) {
  ...
  TPE_Vec3 a = TPE_vec3Minus(size,shifted);TPE_Vec3 b = TPE_vec3Plus(shifted,size);

  byte sx = 1;byte sy = 1;byte sz = 1;
...
void TPE_bodyGetAABB(const(TPE_Body)* body, TPE_Vec3* vMin, TPE_Vec3* vMax) {
  ...
  *vMax = *vMin;
  ...
  for (ushort i = 1; i < body.jointCount; ++i)
  {
    TPE_Unit v;
...
TPE_Vec3 TPE_castEnvironmentRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, TPE_ClosestPointFunction environment, TPE_Unit insideStepSize, TPE_Unit rayMarchMaxStep, uint maxSteps) {
  ...
  ubyte found = 0; // 0 = nothing found, 1 = out/in found, 2 = in/out found
  ...
  if (found)
  {
    ...
    for (ubyte i = 0; i < 128; ++i) // upper limit just in case
    {
...
TPE_Vec3 TPE_castBodyRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, short excludeBody, const(TPE_World)* world, short* bodyIndex, short* jointIndex) {
  ...
  for (ushort i = 0; i < world.bodyCount; ++i)
  {
    TPE_Vec3 c;TPE_Vec3 p;
    TPE_Unit r;TPE_Unit d;
        ...
        for (ushort j = 0; j < world.bodies[i].jointCount; ++j)
        {
          ...
          if (TPE_vec3Dot(p,rayDir) >= 0)
          {
            ...
            if (d <= js)
            {
              ...
              TPE_Vec3 i1 = TPE_vec3Plus(p,c);TPE_Vec3 i2 = TPE_vec3Minus(p,c);
...
TPE_Vec3 TPE_envAATriPrism(TPE_Vec3 point, TPE_Vec3 center, const(TPE_Unit)[6] sides, TPE_Unit depth, ubyte direction) {
  ...
  for (ubyte i = 0; i < 6; i += 2)
  {
    ubyte i2 = i < 4 ? i + 2 : 0;

    TPE_Vec3 p = TPE_envHalfPlane(point,TPE_vec3(sides[i],sides[i + 1],0),
...
uint TPE_jointHash(const(TPE_Joint)* joint) {
  uint r = _TPE_hash(joint.position.x);
    r = _TPE_hash(r ^ joint.position.y);
    r = _TPE_hash(r ^ joint.position.z);
    r = _TPE_hash(r ^
      ((cast(uint) joint.velocity[0]) |
      ((cast(uint) joint.velocity[1]) << 16)));
    r = _TPE_hash(r ^ 
      ((cast(uint) joint.velocity[2]) |
      (cast(uint) joint.sizeDivided)));
...
uint TPE_connectionHash(const(TPE_Connection)* connection) {
  return _TPE_hash(
   (cast(uint) connection.length) |
   ((cast(uint) connection.joint1) << 16) |
   ((cast(uint) connection.joint2) << 24));
}

uint TPE_bodyHash(const(TPE_Body)* body) {
  uint r = _TPE_hash(
    (cast(uint) body.jointMass) |
    ((cast(uint) body.flags) << 16) |
    ((cast(uint) body.deactivateCount) << 24)) ^
      _TPE_hash(
    (cast(uint) body.friction) |
    ((cast(uint) body.elasticity) << 16));

  for (ubyte i = 0; i < body.jointCount; ++i)
    r = _TPE_hash(r ^ TPE_jointHash(&body.joints[i]));

  for (ubyte i = 0; i < body.connectionCount; ++i)
...
uint TPE_worldHash(const(TPE_World)* world) {
  ...
  for (ubyte i = 0; i < world.bodyCount; ++i)
...
ubyte TPE_testClosestPointFunction(TPE_ClosestPointFunction f, TPE_Vec3 cornerFrom, TPE_Vec3 cornerTo, ubyte gridResolution, TPE_UnitReduced allowedError, TPE_Vec3* errorPoint) {
  ...
  for (ushort z = 0; z < gridResolution; ++z)
  {
    p.z = cornerFrom.z + (z * cornerTo.z) / gridResolution;

    for (ushort y = 0; y < gridResolution; ++y)
    {
      p.y = cornerFrom.y + (y * cornerTo.y) / gridResolution;

      for (ushort x = 0; x < gridResolution; ++x)
      {
...
TPE_Vec3 TPE_envHeightmap(TPE_Vec3 point, TPE_Vec3 center, TPE_Unit gridSize, TPE_Unit function(int x, int y) heightFunction, TPE_Unit maxDist) {
  ...
  short startSquareX = point.x / gridSize - (point.x < 0);short startSquareY = point.z / gridSize - (point.z < 0);

  short squareX = startSquareX;short squareY = startSquareY;

  ubyte spiralDir = 1;
  ushort spiralStep = 1;ushort spiralStepsLeft = 1;

  TPE_Vec3 bl = TPE_vec3(squareX * gridSize,heightFunction(squareX,squareY),
      squareY * gridSize);TPE_Vec3 br = TPE_vec3(bl.x + gridSize,heightFunction(squareX + 1,squareY),bl.z);TPE_Vec3 tl = TPE_vec3(bl.x,heightFunction(squareX,squareY + 1),bl.z + gridSize);TPE_Vec3 tr = TPE_vec3(br.x,heightFunction(squareX + 1,squareY + 1),tl.z);

  for (ushort i = 0; i < 1024; ++i) // while (1) should work in theory but...
  {
    ...
    for (ubyte j = 0; j < 2; ++j) // check the two triangles of the segment
    {
...
enum string testEdge(string a,string b) = ` \
